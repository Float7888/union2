...
export class FirecrawlService {
  ...
  static async scrapeJobDetails(jobUrl: string): Promise<{ success: boolean; error?: string; data?: any }> {
    const apiKey = this.getApiKey();
    if (!apiKey) {
      return { success: false, error: 'API key not found' };
    }
    ...
    try {
      ...
      const scrapeResponse = await this.firecrawlApp.scrapeUrl(jobUrl, {
        formats: ['markdown'],
      ...
      if (!scrapeResponse.success) {
        ...
        return { 
          success: false, 
          error: scrapeResponse.error || 'Failed to scrape job details' 
        };
      }
